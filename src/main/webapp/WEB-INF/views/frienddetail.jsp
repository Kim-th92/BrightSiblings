...
<head>
<meta charset="UTF-8">
<title>${dto.member_name}님의페이지입니다.</title>
					...
					<ul>
						...
						<c:choose>
							<c:when test="${res eq 1}">
								<li><button onclick="" class="friend">
										<i class=" fab fa-facebook-messenger"></i>메세지보내기
									</button>
									<button onclick="" class="friend">
										<i class=" fas fa-user-minus"></i>친구차단
									</button></li>
							</c:when>
							<c:otherwise>
								<li><button onclick="friendRequest(${dto.member_no});"
										id="request-friend">
										<i class="fas fa-user-plus"></i>친구추가
									</button></li>
							</c:otherwise>
						</c:choose>
