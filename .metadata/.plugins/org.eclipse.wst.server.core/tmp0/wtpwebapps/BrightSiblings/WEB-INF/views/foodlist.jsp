	...
	<div style="width: 100%;" id="fooddict-wraper">
		...
		<div style="text-align: center; margin: 20px 0px;">
			<img style="width: 90%; height: 150px;"
				src="./resources/image/foodbanner.jpg" alt="음식배너" />
		</div>
		<div class="table-responsive" style="margin: auto; width: 80%;">
			...
			<div style="display: block; text-align: center;">
				<c:if test="${paging.startPage != 1 }">
					<a style="color: #ff1a8c;"
						href="foodlist.do?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
				</c:if>
				<c:forEach begin="${paging.startPage }" end="${paging.endPage }"
					...
					<c:choose>
						...
						<c:when test="${p != paging.nowPage }">
							<a style="color: #ff1a8c;"
								href="foodlist.do?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
						</c:when>
					</c:choose>
				</c:forEach>
				<c:if test="${paging.endPage != paging.lastPage}">
					<a style="color: #ff1a8c;"
						href="foodlist.do?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}">&gt;</a>
				</c:if>
			</div>
			<!-- pagination{e} -->
			...
			<div class="form-group row justify-content-center">
				...
				<div>
					<button class="btn btn-sm btn-primary"
						style="background-color: #ff1a8c; border: none;" name="btnSearch"
